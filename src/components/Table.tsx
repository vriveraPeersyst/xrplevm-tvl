    ...
    <table className="min-w-full">
      ...
      <tbody>
        {loading ? (
          ...
          const isHighlighted = r.key === highlightedKey;
          return (
            <tr
              key={r.key}
              className={`border-b border-black/30 even:bg-black/10 text-center group cursor-pointer transition-all duration-200 ${isHighlighted ? 'bg-lightPurple/20 !z-30' : ''}`}
              style={isHighlighted ? {
                position: 'relative',
                zIndex: 30,
